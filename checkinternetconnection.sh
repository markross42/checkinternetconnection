...
# reachable by the bradband router. So ping times less than 10ms are internal -> FAIL
MIN_TIME=10.0
...
do
  ...
  if [ $RESULT_CODE -ge 1 ] && [ $FAILURE -eq 0 ] # the ping was successfull
    then
    FAILURE=1
    echo "$DATETIME;Start failure;$TIME;PING_FAILED"
    echo "$DATETIME;Start failure;$TIME;PING_FAILED" >> $LOG_FILE
  fi
  
  if [ $RESULT_CODE -eq 0 ] && [ $FAILURE -eq 0 ] # the ping was successfull
  then
    #if (( $(echo "$TIME < $MIN_TIME" | $BC_CMD ) )) # was the ping in the internal or external network?
    PING_TOO_QUICKLY=$(echo "$TIME < $MIN_TIME" | bc -l)
    if [ $PING_TOO_QUICKLY -eq 1 ] # was the ping in the internal or external network?
    then
      FAILURE=1
      echo "$DATETIME;Start failure;$TIME;TOO_QUICKLY"
  ...
  then
    ...
    then
      #FAILURE=1
      echo "$DATETIME;Failure exists;$TIME;TOO_QUICKLY"
      #echo $DATETIME ";Start failure;$TIME;TOO_QUICKLY" >> $LOG_FILE
    else
      FAILURE=0
    echo "$DATETIME;End failure;$TIME;PING_OK"
    echo "$DATETIME;End failure;$TIME;PING_OK" >> $LOG_FILE
    fi
  fi
  sleep $WAITING
done